...
\begin{enumerate}
    ...
    \item Il sistema permetterà ad utenti amministratori di assegnare \index{Issue!Assegnazione issue} issue specifiche ad un membro del team.
    \item Il sistema potrà inviare notifiche agli utenti.
    \item Il sistema genererà report mensili sull'attività del team.
    \item Il sistema permetterà agli amministratori di visualizzare \index{Vista!Report mensile} i report sul team.
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \hyperref[sec:RE12]{Registrazione nuova utenza (Utente):} Un utente non registrato si registra al sistema.\index{Nuova utenza (Utente)} 
    ...
    \begin{itemize}
        \item \hyperref[sec:RE03]{Segnala issue:}\index{Issue!Segnalazione}\index{Issue!Filtra}\index{Issue!Ordina}\index{Issue!Ricerca} Un utente autenticato può segnalare una issue, della quale deve inserire un titolo e una descrizione. Può anche aggiungere immagini allegate e impostare una priorità.
        \item \hyperref[sec:RE04]{Assegna issue:}\index{Assegna} Un amministratore può assegnare una issue ad un membro del team (utente).
        \item \hyperref[sec:RE11]{Modifica stato issue:} Un utente cui è stata assegnata una issue ne modifica lo stato.\index{Issue!Modifica} 
    \end{itemize}
    ...
    \begin{itemize}
        \item \hyperref[sec:RE05]{Visualizza issue:}\index{Vista!Riepilogo isseu} gli utenti autenticati possono visualizzare la lista delle issue.
...
È un membro del team di sviluppo software. Può autenticarsi per segnalare nuove issue, visualizzarle e modificare quelle che gli sono state
assegnate. & Conoscenza base del dominio del software. Competenze basilari di utilizzo di applicativi software. & Segnala issue; visualizzare riepilogo issue; 
...
Effettuare il log in; registrare nuova utenza \\ \hline
\end{tabularx}
...
Precondizione & L'utenza registrata \\ \hline
Effetti collaterali & Non ci sono effetti collaterali \\ \hline 
...
Use Case & \hyperref[sec:UC05]{Vedi Use Case relativo a RE02} \\ \hline
\end{tabularx}
...
impostare una priorità e allegare immagini \\ \hline
Input & Titolo, descrizione e opzionalmente immagini e priorità \\ \hline
...
Descrizione & Il sistema deve permettere di assegnare issue a membri del team \\ \hline
...
Precondizione & L'utente deve essere autenticato con permessi di amministratore e la issue non deve essere stata già assegnata ad altri utenti \\ \hline
Effetti collaterali & Il sistema deve inviare una notifica all'utente cui è stata assegnata la issue e deve modificare lo 
...
Richiede & Che sia presente almeno una issue nel sistema \\ \hline
Precondizione & L'utente deve essere autenticato \\ \hline
...
Richiede & Che sia presente almeno una issue nel sistema assegnata ad un membro del team per il mese selezionato \\ \hline
Precondizione & L'utente deve essere autenticato con permessi di amministratore \\ \hline
Effetti collaterali & Nessuno \\ \hline 
...
Richiede & Che sia presente almeno una issue assegnata all'utente che fa richiesta della lista \\ \hline
Precondizione & L'utente deve essere autenticato \\ \hline
Effetti collaterali & Nessuno \\ \hline
...
\begin{itemize}
    ...
    \item Priorità
    \item Data
    \item Utente che l'ha segnalata
\end{itemize}
\\ \hline
...
\subsubsection{RE011 - Modifica stato issue}\index{Issue!Modifica}
\label{sec:RE11}
...
\begin{tabularx}{\textwidth}{p{5cm}|X}
Funzione & Modifica lo stato di una issue \\ \hline
Descrizione & Il sistema deve permettere all'utente autenticato cui è stata assegnata una issue di modificarne lo stato da \textit{In Progress} a \textit{Done}, 
una volta risolta \\ \hline
Input & Selezione modifica issue \\ \hline
Fonte & Interfaccia utente nella vista di visualizzazione delle proprie issue \\ \hline
Output & Stato issue modificato \\ \hline
Azione & Il sistema deve modificare il valore dello stato della issue da \textit{In Progress} a \textit{Done} \\ \hline
Precondizione & L'utente deve essere autenticato, deve avere almeno una issue assegnata e lo stato della issue deve essere \textit{In Progress}. \\ \hline
Effetti collaterali & Il sistema invia una notifica all'utente che ha segnalato la issue, avvertendolo che il problema è stato risolto. \\ \hline
Use Case & \hyperref[sec:UC06]{Vedi Use Case Modifica stato issue - RE06} \\ \hline
\end{tabularx}

\subsubsection{RE012 - Nuova utenza (Utente)}\index{Nuova utenza (Utente)}
\label{sec:RE12}
...
\begin{tabularx}{\textwidth}{p{5cm}|X}
Funzione & Registrazione nuova utenza \\ \hline
Descrizione & Il sistema deve permettere di registrare nuove utenze
all’interno del sistema software. \\ \hline
Input & Email e password \\ \hline
Fonte & Interfaccia utente \\ \hline
Output & Visualizza messaggio di avventua registrazione della nuova
utenza o visualizza messaggio di errore \\ \hline
Azione & Attivazione della nuova utenza con possibilità di autenticazione \\ \hline
Richiede & Credenziali conformi alle regole imposte dal sistema software \\ \hline
Precondizione & Nessuna \\ \hline
Effetti collaterali & Nessuno \\ \hline
Use Case & Incluso in \hyperref[sec:UC01]{Log in - RE01} \\ \hline
\end{tabularx}

\subsection{Requisiti non funzionali}
Questa sezione è lasciata momentaneamente vuota.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/BugBoard26.png}
...
I casi d'uso mostrati sono due: \textit{Log in e Registra nuova utenza (Utente)}. 
\begin{itemize}
...
registrato e attivo nel sistema. 
    \item \textit{Registra nuova utenza (Utente)}. L'attore principale è un utente non registrato. Esso può registrarsi al sistema
specificando una email e una password. 
\end{itemize} 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Parziali/BugBoard26_login.png}
    \caption{Use Case1 - \hyperref[sec:RE01]{Requisito funzionale 3.1.1 - Log in utente} e \hyperref[sec:RE12]{Requisito funzionale 3.1.12 -  Crea nuova utenza (Utente)}}
    \label{fig:etichetta}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Parziali/BugBoard26_SegnalaIssue.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Parziali/BugBoard26_Visualizza&Assegna.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Parziali/BugBoard26_VisualizzaReport&VisualizzaRiepilogo.png}
...
\subsubsection{Use Case Diagram parziale 5}\index{Use Case!Creazione nuova utenza (Amministratore)}
\label{sec:UC05}
Il seguente Use Case è un estratto dello Use Case generale. Non aggiunge informazioni al caso d'uso specifico. Serve solamente a focalizzare l'attenzione sul caso/i d'uso specifico/i.
In particolare, si pone l'attenzione sulla registrazione di una nuova utenza da parte di un amministratore. L'attore principale è un amministratore. Esso può inserire nel sistema un nuovo utente. 
Deve specificare le credenziali di accesso (email e password) e impostare una tipologia all'utenza (utente normale o amministratore). 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Parziali/BugBoard26_CreaNuovaUtenza.png}
    \caption{Use Case5 - \hyperref[sec:RE02]{Requisito funzionale 3.1.2 -  Crea nuova utenza (Amministratore)}}
    \label{fig:etichetta}
\end{figure}

\subsubsection{Use Case Diagram parziale 6}\index{Use Case!Modifica stato issue}
\label{sec:UC06}
Il seguente Use Case è un estratto dello Use Case generale. Non aggiunge informazioni al caso d'uso specifico. Serve solamente a focalizzare l'attenzione sul caso/i d'uso specifico/i.
In particolare, si pone l'attenzione sulla modifica dello stato di una issue. L'attore principale è un utente autenticato. Esso può modificare lo stato di una issue 
assegnatagli, portandola dallo stato \textit{In Progress} allo stato \textit{Done}. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/Parziali/BugBoard26_ModificaStatoIssue.png}
    \caption{Use Case6 - \hyperref[sec:RE11]{Requisito funzionale 3.1.11 - Modifica stato issue}}
    \label{fig:etichetta}
\end{figure}
...
\hline
\textbf{Use Case x} & \textbf{Nome UC} \\ \hline
Goal in Context & Permette agli utenti autenticati di creare una issue con titolo e descrizione. Facoltativamente, 
di inserire un'immagine in allegato e assegnare un livello di priorità  \\ \hline
...
3 & Inserisce i campi obbligatori: titolo descrizione e tipo di Issue \textit{(question, bug, documentation, feature)} & \\ \hline
4 & Clicca \textit{Crea} &  \\ \hline
5 & & \hyperref[sec:MU02]{Mostra MU\_2} \\ \hline
6 & Clicca \textit{Si} & \\ \hline
7 &  & Crea la issue e \hyperref[sec:MU03]{Mostra MU\_3} \\ \hline
\end{tabularx}
\end{minipage}
...
4.a & & Torna allo step 4 del \textit{Main Scenario}
\end{tabularx}
...
\begin{tabularx}{\linewidth}{p{0.8cm}|X|X}
\textbf{Step} & \textbf{Utente autenticato} & \textbf{Risposta del sistema} \\ \hline
3.b & Clicca su \textit{Allega img} &  \\ \hline
4.b & Clicca \textit{Crea} & \\ \hline
5.b & & Consente la scelta dell'immagine e torna allo step 4 del \textit{Main Scenario} 
\end{tabularx}
\end{minipage}\strut \\ \hline

Extension \textit{\#3} - Gestione Errore caricamento immagine  & 
\begin{minipage}[t]{\linewidth}
\begin{tabularx}{\linewidth}{p{0.8cm}|X|X}
\textbf{Step} & \textbf{Utente autenticato} & \textbf{Risposta del sistema} \\ \hline
3.b.1 & Clicca su \textit{Allega img} &  \\ \hline
4.b.1 & & \hyperref[sec:MU04]{Mostra MU\_4} e torna allo step 4 del \textit{Main Scenario}
\end{tabularx}
\end{minipage}\strut \\ \hline
...
5.c & & \hyperref[sec:MU05]{Mostra MU\_5} e torna allo step 3 del \textit{Main Scenario}
\end{tabularx}
\end{minipage}\strut \\ \hline
...
\begin{tabularx}{\linewidth}{p{0.8cm}|X|X}
\textbf{Step} & \textbf{Utente autenticato} & \textbf{Risposta del sistema} \\ \hline
6.e & Clicca \textit{No} &  \\ \hline
7.e & & Torna allo step 3 del \textit{Main Scenario}
\end{tabularx}
\end{minipage}\strut \\ \hline
...
\begin{figure}[H]
    ...
    \caption{Mockup 3 - Crea issue riferito a \hyperref[sec:CKB01]{3.4.1 Tabella di Cockburn 1 - Step 7}}
    \label{fig:etichetta}
\end{figure}
...
\subsubsection{Segnala Issue - Mockup 5}
\label{sec:MU05}
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/SegnalaIssueMockup/MU_5.png}
    \caption{Mockup 5 - Crea issue riferito a \hyperref[sec:CKB01]{3.4.1 Tabella di Cockburn 1 - Step 5.c}}
    \label{fig:etichetta}
\end{figure}
...
\subsubsection{Descrizione dello schema di persistenza dei dati}
