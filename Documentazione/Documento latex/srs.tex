...
% Piè di pagina
\fancyfoot[L]{\textcolor{gray}{Documentazione - \textit{BugBoard26}} \hfill\thepage}
\fancyfoot[C]{}
\fancyfoot[R]{}
...
\title{\textbf{Documentazione Bugboard26} \\ \large Progetto Ingegneria del Software 2025/2026  }
...
\begin{itemize}
    ...
    \item \textbf{Metrica}: unità di misura quantitativa utilizzata per valutare l'efficienza complessiva del team di sviluppo. Le metriche integrate sono: Numero di issue segnalate, numero di issue gestite, tempo di risoluzione medio e carico di lavoro attuale.
    ...
    \item \textbf{Notifica}: messaggio automatico generato con cui la piattaforma comunica ad utenti autenticati l'avvenuta assegnazione di una issue.
    ...
    \item \textbf{Report}: visualizzazione di riepilogo generato mensilmente che sintetizza dati aggregati e statistiche sulle attività svolte e sulla produttività del team (riservato agli amministratori).
...
La restante parte di questo documento è suddivisa in cinque ulteriori capitoli che descrivono le caratteristiche funzionali di \textit{BugBoard26}.\\
...
In \hyperref[sec:testPlan]{\textbf{"6. Test Plan"}} vengono descritti in dettaglio i test effettuati.
\begin{longtable}{|p{5cm}|p{9cm}|} 
\hline 
Test per la funzionalità 4 - Assegna Issue & Test Plan completo per l'intera funzionalità \\ \hline 
Test del metodo \texttt{AssignIssue} & Descrizione della classe di test per il metoso \texttt{AssignIssue} \\ \hline 
Test del metodo \texttt{createNotification} & Descrizione della classe di test per il metodo \texttt{createNotification} \\ \hline 
Test del metodo \texttt{getMonthlyReport} & Descrizione della classe di test per il metodo \texttt{createNotification} \\ \hline 
\end{longtable}
...
\begin{itemize}
    \item \textbf{Categoria Autenticazione}\index{Autenticazione} - \textit{BugBoard26} consente l'autenticazione dell'utente, consentedogli di accedere solo alle funzionalità che gli competono.
    \begin{itemize}
        \item \hyperref[sec:RE01]{Autenticazione:} \textit{BugBoard26} consente l'autenticazione dell'utente.
    ...
    \end{itemize}
    \item \textbf{Categoria issue}\index{Issue} - \textit{BugBoard26} gestisce la segnalazione e l'assegnazione di issue.
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Categoria visualizzazioni}\index{Vista} -  \textit{BugBoard26} permette di visualizzare informazioni su issue e membri del team.
    \begin{itemize}
        ...
        \item\hyperref[sec:RE11]{Visualizza notifiche:} \index{Vista!Notifiche} Il sistema fornisce una dashboard per la visualizzazione e gestione delle proprie notifiche.
    \end{itemize}
    \item \textbf{Categoria Notifiche}\index{Notifiche} -  \textit{BugBoard26}  gestisce l'invio di notifiche agli utenti.
    \begin{itemize}
        \item \hyperref[sec:RE04]{Invio notifiche:} \index{Notifiche!Invio} Il sistema notifica un utente quando un amministratore gli assegna una nuova issue.
        \item \hyperref[sec:RE12]{Lettura notifiche} \index{Notifiche!Leuttura} Un utente può segnare le proprie notifiche come lette o come non lette.
    \end{itemize}
\end{itemize}
...
Precondizione & L'utente deve essere autenticato. \\ \hline
Effetti collaterali & Nessuno \\ \hline
Use Case & Vedi Use case relativo a \hyperref[sec:UC06]{Visualizza elenco notifiche - RE11} \\ \hline
\end{tabularx}
...
Precondizione & L'utente deve essere autenticato. \\ \hline
Effetti collaterali & Nessuno \\ \hline
Use Case & Incluso in \hyperref[sec:UC06]{Leggi notifiche - RE12} \\ \hline
\end{tabularx}
...
Effetti collaterali & Nessuno \\ \hline
Use Case & Incluso in \hyperref[sec:UC03]{Workload - RE13} \\ \hline
\end{tabularx}
...
\begin{figure}[H]
    ...
    \caption{Use Case3 - \hyperref[sec:RE04]{Requisito funzionale 3.1.4 - Assegna issue}, \hyperref[sec:RE07]{Requisito funzionale 3.1.7 - Visualizza issue assegnate} e \hyperref[sec:UC03]{Requisito funzionale 3.1.13 - Workload}} 
    \label{fig:etichetta}
\end{figure}
...
\subsection{Prototipazione visuale del caso d'uso Segnala Issue}
\index{Mockup}
...
\begin{itemize}
    ...
    \item Business Logic Layer (Server)
...
\subsubsection{Class Diagram 2- Funzionalità 2}Il seguente class diagram rappresenta le classi coninvolte nella funzionalità 2
\begin{figure}[H]
    \centering
...
\subsubsection{Class Diagram 3- Funzionalità 3}Il seguente class diagram rappresenta le classi coninvolte nella funzionalità 3
\begin{figure}[H]
    \centering
...
\subsubsection{Class Diagram 4- Funzionalità 4 e 14}Il seguente class diagram rappresenta le classi coninvolte nelle funzionalità 4 e 14
\begin{figure}[H]
    \centering
...
\subsubsection{Class Diagram 5- Funzionalità 11}Il seguente class diagram rappresenta le classi coninvolte nella funzionalità 11
\begin{figure}[H]
    \centering
...
\subsubsection{Class Diagram 6- Funzionalità 17}Il seguente class diagram rappresenta le classi coninvolte nella funzionalità 17
\begin{figure}[H]
    \centering
...
Per avere un basso accoppiamento e un'alta coesione nelle classi \texttt{ViewController} del \textit{frontend} si è spostata la logica di presenzazione grafica ad alcune classi di supporto (\texttt{Utils}), alle quali i \texttt{ViewController} delegano la creazione di tabelle, grafici, badge, ect. Tutti gli stili grafici sono raggruppati in una classe CSS \texttt{style.css}. 
 

\subsection{Strumento di Versioning} Per la gestione del versionamento del codice sorgente e della documentazione è stato utilizzato \textbf{GitHub}. È stato adottato un unico repository per l'intero progetto, organizzato nelle seguenti directory principali:\index{GitHub}
\begin{itemize}
...
Di seguito sono riportati i dati dell’ultima modifica effettuata sul Server, il report si basa su un'indagine su un totale di 1.8k righe di codice. \index{SonarQube}
...
È stato scelto il metodo \texttt{assignIssue(UUID issueUuid, UUID userUuid)} della classe \texttt{IssueWriteService}. Il metodo ha il compito di assegnare una issue ad un utente.

Il comportamento previsto è:
\begin{enumerate}
    \item Recuperare l'issue tramite \texttt{issueUuid}
    \item Recuperare l'utente tramite \texttt{userUuid}
...
Per il testing è stato adottato un approccio di \textbf{unit testing} con Mockito. Sono stati utilizzati mock per simulare il comportamento delle seguenti classi: \texttt{IssueRepository}, \texttt{UserRepository}, \texttt{AuthService}, \texttt{IssueMapper}, \texttt{ApplicationEventPublisher}.
...
Test JUnit & \texttt{testAssignIssueValidUser()} \\ \hline
\end{longtable}
...
Test JUnit & \texttt{testAssignIssueUserNotFound()} \\ \hline
\end{longtable}
...
Output atteso (Oracle) & Il metodo deve lanciare \texttt{ResourceAccessException} \\ \hline
Test JUnit & \texttt{testAssignIssueIssueNotFound()} \\ \hline
\end{longtable}

\subsubsection{White box}
Analizzando il codice sorgente di \texttt{assignIssue}, i soli punti di decisione (branch) presenti sono i due controlli \texttt{orElseThrow} su \texttt{issueRepository.findByUuid(...)} e \texttt{userRepository.findByUuid(...)}; il resto del metodo è codice sequenziale privo di condizioni.

\begin{longtable}{|p{9cm}|p{2cm}|}
\hline
\textbf{Ramo} & \textbf{Coperto} \\ \hline
Issue trovata (TC-AI1, TC-AI2) & \checkmark \\ \hline
Issue non trovata (TC-AI3) & \checkmark \\ \hline
Utente trovato (TC-AI1) & \checkmark \\ \hline
Utente non trovato (TC-AI2) & \checkmark \\ \hline
\end{longtable}

I 3 casi di test esistenti raggiungono quindi la copertura completa dei branch effettivi del metodo (100\% branch coverage).

\textit{Nota:} \texttt{AuthService} e \texttt{IssueMapper} sono dipendenze del costruttore di \texttt{IssueWriteService} (usate dal metodo \texttt{createIssue}, non documentato in questa sezione), ma \textbf{non vengono mai richiamate all'interno di \texttt{assignIssue}}. Sono mockate solo perché necessarie per istanziare la classe tramite \texttt{@InjectMocks}, non perché il metodo sotto test le utilizzi: non rappresentano quindi rami di codice non coperti, semplicemente non sono pertinenti a questo metodo.
...
\hline
\textbf{Classe} & \textbf{Descrizione} & \textbf{Valore rappresentativo} \\ \hline
CE2 & User valido esistente nel sistema & User presente e valido \\ \hline
\end{longtable}
...
\hline
\textbf{Classe} & \textbf{Descrizione} & \textbf{Valore rappresentativo} \\ \hline
CE3 & Salvataggio corretto della notifica & Repository funzionante \\ \hline
CE4 & Fallimento del salvataggio & \texttt{RuntimeException} dal repository \\ \hline
\end{longtable}
...
Test JUnit & \texttt{testCreateNotificationValid()} \\ \hline
\end{longtable}
...
Test JUnit & \texttt{testCreateNotificationWhenRepositoryThrowsException()} \\ \hline
\end{longtable}
...
Il test reale utilizza un \texttt{ArgumentCaptor} per catturare l'oggetto \texttt{Notification} istanziato internamente dal metodo, anziché un semplice \texttt{any()}:
\begin{lstlisting}
...
Il comportamento previsto è:
\begin{enumerate}
    \item Calcolare il range temporale del mese tramite \texttt{LocalDateTime.of(year, month, 1, 0, 0)} (\texttt{startDate}) e \texttt{startDate.plusMonths(1)} (\texttt{endDate})
...
Test JUnit & \texttt{testGetMonthlyReportValid()} \\ \hline
\end{longtable}
...
Output atteso (Oracle) & Il metodo restituisce lista vuota; il repository viene comunque chiamato correttamente; il mapper non viene mai invocato; nessuna eccezione viene lanciata \\ \hline
Test JUnit & \texttt{testGetMonthlyReportEmptyList()} \\ \hline
\end{longtable}

\paragraph{TC-GMR3 -- Mese non valido, limite superiore (BVA4)}
\label{sec:TC-GMR3}
\begin{longtable}{|p{4cm}|p{10.3cm}|}
\hline
Input & year = 2024, month = 13 \\ \hline
Output atteso (Oracle) & Il metodo deve lanciare \texttt{DateTimeException} \\ \hline
Test JUnit & \texttt{testGetMonthlyReportInvalidMonthUpperBound()} \\ \hline
\end{longtable}

\paragraph{TC-GMR4 -- Mese non valido, limite inferiore (BVA3)}
\label{sec:TC-GMR4}
\begin{longtable}{|p{4cm}|p{10.3cm}|}
\hline
Input & year = 2024, month = 0 \\ \hline
Output atteso (Oracle) & Il metodo deve lanciare \texttt{DateTimeException} \\ \hline
Test JUnit & \texttt{testGetMonthlyReportInvalidMonthLowerBound()} \\ \hline
\end{longtable}

\paragraph{TC-GMR5 -- Mese al limite inferiore valido (BVA1)}
\label{sec:TC-GMR5}
\begin{longtable}{|p{4cm}|p{10.3cm}|}
\hline
Input & year = 2024, month = 1 \\ \hline
Precondizioni & Il repository restituisce lista vuota \\ \hline
Output atteso (Oracle) & Il metodo non lancia eccezioni; viene restituita una lista vuota; il repository viene chiamato con il range corretto (1 gennaio -- 1 febbraio 2024) \\ \hline
Test JUnit & \texttt{testGetMonthlyReportBoundaryMinMonth()} \\ \hline
\end{longtable}

\paragraph{TC-GMR6 -- Mese al limite superiore valido (BVA2)}
\label{sec:TC-GMR6}
\begin{longtable}{|p{4cm}|p{10.3cm}|}
\hline
Input & year = 2024, month = 12 \\ \hline
Precondizioni & Il repository restituisce lista vuota \\ \hline
Output atteso (Oracle) & Il metodo non lancia eccezioni; viene restituita una lista vuota; il repository viene chiamato con il range corretto (1 dicembre 2024 -- 1 gennaio 2025) \\ \hline
Test JUnit & \texttt{testGetMonthlyReportBoundaryMaxMonth()} \\ \hline
\end{longtable}
...
verify(userReportMapper, never())
    .mapToDTO(any());
\end{lstlisting}

\textbf{Controllo del caso input non valido:}
Lo stesso pattern è applicato sia al limite superiore (month = 13) sia al limite inferiore (month = 0):
\begin{lstlisting}
assertThrows(
